...
- [ ] **`orchestrator/docker_client.py:72`** — `list_images()`: remove `prefix`
...
- [ ] **`orchestrator/models.py:146–161`** — `ImageSummary.from_docker()`:
...
- [ ] **`orchestrator/models.py:169–`** — `ImageDetail.from_docker_inspect()`:
...
- [ ] **`orchestrator/container_manager.py:74–76`** — `ImageNotFound.__init__`:
  change error message from `f"Image 'drover/{image}' not found"` to
  `f"Image '{image}' not found"`.

- [ ] **`orchestrator/container_manager.py:285–295`** — container creation
...
- [ ] **`orchestrator/routers/images.py:16–23`** — `get_image(name)`: remove
...
- [ ] **`tests/test_models.py:176`** — update `ImageSummary.from_docker()` test
...
- [ ] **`tests/test_models.py:195`** — same fixture update for the second
  `ImageSummary` test case.

- [ ] **`tests/test_container_manager.py:94`** — update assertion: was
  `docker.inspect_image.assert_called_once_with("drover/python-runner")`;
  change to assert the new label-lookup behaviour instead.

- [ ] **`tests/test_container_manager.py:140`** — update comment about
  privileged images not using the `drover/` image.

### Webapp — UI

- [ ] **`webapp/src/views/partials/images-list.js:37`** — update the empty-state
...
- [ ] **`webapp/test/proxy.test.js:159`** — `image: 'drover/example'`
- [ ] **`webapp/test/orchestrator.test.js:86`** — `{ image: 'drover/x' }`
- [ ] **`webapp/test/views.test.js:38`** — `image: 'drover/python-runner'`
- [ ] **`webapp/test/views.test.js:50`** — `image: 'drover/node-runner'`
- [ ] **`webapp/test/actions.test.js:134`** — `image: 'drover/python-runner'`

### Builder image

- [ ] **`builder/Dockerfile`** — add required labels:
  ```dockerfile
...
- [ ] **`README.md:34`** — update prerequisite from "tagged with the `drover/`
  prefix" to describe the required labels.

- [ ] **`README.md:148`** — update privileged container note (currently says
  "does not use a `drover/`-prefixed image").

- [ ] **`README.md:198–206`** — rewrite the Image Management / Naming Convention
...
- [ ] **`README.md:212`** — update "List all `drover/*` images" to reflect
  label-based listing.

- [ ] **`README.md:241`** — update "validates that `drover/<image>` exists" to
  describe label-based lookup.

- [ ] **`orchestrator/README.md:112`** — update `image` field description;
  remove "Resolved as `drover/<image>`" note; explain that the value must match
  a `drover.name` label on an installed image.

- [ ] **`orchestrator/README.md:141`** — update endpoint description from "List
  all drover/* images" to describe label-based filtering.

- [ ] **`orchestrator/README.md:145`** — rewrite the images naming section to
  describe the label contract.

- [ ] **`docs/decisions/2026-04-16-drover-templates.md`** — update all
