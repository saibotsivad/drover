...
- `orchestrator/docker_client.py:143` already has a non-streaming `get_container_logs()`; a streaming variant is added by this plan.
...
This plan feeds the Docker follow stream to a disk writer so that container logs are no longer ephemeral.
...
For each container that reaches `running`, **if** `DROVER_LOG_DIR` is set the orchestrator opens a follow stream via the Docker API:
...
A new `LogCaptureManager` parses this stream and appends one JSON line per parsed `{stream, data, time}` chunk to the current open log file for that container.

When `DROVER_LOG_DIR` is unset, the follow stream is not opened, no directory is created, and no `.cursor` file is maintained.
...
| `initializing`, immediately after `docker.start_container()` succeeds in `_init_container` | If `DROVER_LOG_DIR` is set, create `{DROVER_LOG_DIR}/{id}/`, open `0.log`, and start the capture task using the freshly-known Docker ID. **This happens before the `ready` message and covers the init-failure window.** |
...
| `running` → `stopping` → `stopped` (explicit stop, idle timeout, or done signal) | Capture task observes the Docker follow stream closing, flushes and closes the current file, then exits cleanly. Directory is retained. |
| `stopped` → `resuming` → `running` | Start a new capture task. Reopen the latest log file (or rotate to a new one if the previous file is over the size threshold) and request only logs `since=<last_recorded_ts>` so we resume without gaps or duplicates. |
| any non-destroyed status → `destroying` → `destroyed` | After Docker confirms removal, if a log directory exists, `rm -rf {DROVER_LOG_DIR}/{id}/`. When `DROVER_LOG_DIR` is unset, no-op. This includes destroying an `initializing` or `error` container — the partial init logs go away with the rest of the container's state. |
| Orchestrator restart (`sync_containers`) | When `DROVER_LOG_DIR` is set, for each row in `running` or `initializing` whose Docker container is still up: restart the capture task using `since=<last_recorded_ts>` from `.cursor` to bridge the gap (or `since=0` if no cursor exists yet, which is the common case for a still-initializing container that crashed before its first log line was written). For `stopped`: nothing to do. For `destroyed`: confirm directory is gone; if not, remove it. Containers that get force-failed to `error` by the post-reconciliation pass have their directories preserved like any other `error` row. When `DROVER_LOG_DIR` is unset, no-op. |
...
This plan does not change any external surface in v1.
...
|---|---|---|
| `DROVER_LOG_DIR` | _(unset; the sample compose stack sets it)_ | Root directory for captured micro-container logs. **When unset, Drover writes nothing to disk and provides no retention.** When set, Drover-managed retention is enabled. |
...
The result: a homelab operator with nothing installed has working log retention by reading the capture directory directly. An operator with Loki gets the same retention plus their dashboards, by adding one scrape config. Neither operator has to install or run anything they didn't already want.
...
- The work saved over a streaming approach is small: opening a follow stream and appending to a file is not significantly more code than calling the non-streaming logs API once.
...
The preferred approach (in the Proposal) is to stream from Docker and append to disk. It costs us a single long-lived task per running container and a small amount of disk per container.
...
10. **Disk-backed historical-query REST API is out of scope for v1.** The new disk capture is invisible to REST in v1 and surfaces only via direct disk access. A follow-up pagination plan will broaden `GET /logs` (and standardize pagination across every list endpoint) before this plan is considered released. See Follow-Up Work.
...
- On a persistent write error (e.g. `ENOSPC`), log one structured error to orchestrator stdout, set an internal `_disk_disabled = True` flag, and stop attempting writes for the rest of this orchestrator process's lifetime.
...
- **`TODO.md`:** remove the "Container log retention" section once this is implemented; add an entry pointing at the follow-up pagination plan as the prerequisite for cutting a release that includes log retention.
...
- Unit test disk-full: arrange a writer to receive `ENOSPC` on its next write, assert one error log is emitted, assert subsequent writes are no-ops.
...
2. Wire into `ContainerManager` lifecycle. Now logs land on disk and are deleted on destroy. The REST endpoint is unchanged.
3. Documentation pass: `docs/observability.md`, `README.md` link, `docker-compose.yml` env-var addition, ADR for the on-disk-format decision.

Steps 1–2 are independently shippable and useful (operators get on-disk retention they can read directly). The follow-up pagination plan is prerequisite to a release that *advertises* log retention as a feature, since users will reasonably expect API access to retained logs.
...
| Disk fills up. | Single orchestrator-level error log on first failed write, then disk writes are disabled for the rest of the orchestrator process lifetime. Recovery is "free up space, restart orchestrator." |
...
| Docker daemon's own log driver buffer is smaller than our capture rate, so logs are lost before we read them. | We open a follow stream the moment Docker `start_container` returns; Docker streams in real time as long as the connection is held. The capture task is high-priority and not gated on anything else. If the asyncio event loop is blocked, capture lags — this is an orchestrator-health concern, not a logging concern. |
...
Rollback: this work is additive and isolatable. If something goes wrong in production, the operator can simply unset `DROVER_LOG_DIR` so Drover writes nothing to disk and behavior matches today's. Reverting the code is also low-risk because no existing data structures change.
...
One piece of work is deliberately deferred out of this plan and into a separate effort, but is a prerequisite to a release that includes log retention as an advertised feature.
...
We will not cut a release that publicly advertises log retention until both this plan and the pagination plan are landed. Operators expecting "I can see my container's old logs" should not need to teach themselves the on-disk format to do it. Until both ship, the new capability lives on `main` but is documented as preview.
