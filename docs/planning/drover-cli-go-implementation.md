...
CI workflows. It slots `cli` into the repo's existing per-project
versioning machinery described in [`docs/versioning.md`](../versioning.md)
and into the umbrella release layer described in
[`docs/releases.md`](../releases.md) and its companion plan
[`umbrella-release-implementation.md`](./umbrella-release-implementation.md).
The umbrella layer changes how end users install the CLI (they fetch
`install.sh` from the umbrella release, not from the per-component CLI
release), and adds one coordination point the CLI release job has to
satisfy: emitting a `cli-release-assets.json` workflow artifact for the
umbrella builder to consume.
...
- Cross-platform behaviour. We trust Go's stdlib here; release builds
  exercise cross-compilation. The umbrella release's `install.sh` is
  tested manually on each target OS as part of the umbrella plan's
  install matrix — not as part of CLI tests.
...
in the Go community for shipping cross-compiled CLI binaries to GitHub
Releases.
...
- **Per-component GitHub Release**: artefacts attached to a release
  named after the pushed `cli-v<version>` tag. Marked with
  `make_latest: false` so the umbrella release continues to own
  `releases/latest`.
- **`cli-release-assets.json`**: a small JSON file in the schema
  defined in section 5 of
  [`umbrella-release-implementation.md`](./umbrella-release-implementation.md),
  generated by post-processing GoReleaser's `dist/artifacts.json`. Both
  attached to the per-component release (as a convenience for direct
  consumers) **and** uploaded as a workflow artifact named
  `cli-release-assets` for the umbrella release job to download.
...
The per-component release also gets human-readable release notes from
GoReleaser, sourced from the newest entry in `cli/CHANGELOG.yml`. This
is redundant with the umbrella's `changes.yml` but gives anyone landing
directly on `cli-v0.2.0` some context.
...
End users install via the umbrella release's `install.sh` — that script
is generated per release by the umbrella workflow, has the matching CLI
binary URLs and SHA-256s baked in, and is signed. The mechanics are
documented in [`docs/releases.md`](../releases.md); this plan deliberately
does not duplicate them. The CLI's only obligation is to publish the
binaries the umbrella installer points at, in the form (`.tar.gz` /
`.zip`) and naming convention that the umbrella generator expects.

Two additional paths for developers and edge cases:

1. **`go install`** (Go-savvy users / CI):
   ```sh
   go install github.com/<owner>/drover/cli/cmd/drover@latest
   ```
   Produces a binary without GoReleaser's version metadata baked in, so
   `drover --version` reports `(devel)` or the tag — acceptable for this
   path.

2. **Manual download** from the per-component `cli-v<version>` GitHub
   Release. Useful in restricted environments or when pinning to a CLI
   version that isn't yet part of an umbrella release.

### CI workflows

Two new workflows, plus a small extension to `push-tag.yml`. Names mirror
the existing `publish-image.yml` convention.
...
2. **`.github/workflows/publish-cli.yml`** — reusable workflow
   (`workflow_call`) that runs GoReleaser. Inputs: `version` (the bare
   semver, e.g. `0.2.0`). Permissions: `contents: write` (per-component
   release), `id-token: write` (cosign keyless signing).

   Responsibilities:
   - Run `goreleaser release` with `make_latest: false`.
   - Generate `cli-release-assets.json` from `dist/artifacts.json` in
     the schema documented in section 5 of
     [`umbrella-release-implementation.md`](./umbrella-release-implementation.md).
   - Attach that JSON to the per-component release.
   - Upload the same JSON as a workflow artifact named
     `cli-release-assets` so the umbrella job can `actions/download-artifact`
     it.
   - Emit `version` as a workflow output (same value as the input),
     so the calling workflow can hand it to `umbrella-release.yml`.

3. **Extension to `.github/workflows/push-tag.yml`** — add a `cli` case
   to the `scan` step alongside `orchestrator`/`builder`/`webapp`, and a
   downstream `publish-cli` job that `uses:` `publish-cli.yml`. The
   umbrella job (added by the umbrella plan) gains `publish-cli` as
   another `needs:` entry and consumes its workflow output as
   `cli_version`. The git tag (`cli-v<version>`) is created the same way
...
|---|---|---|
| `cli` | `cli/` | Per-component GitHub Release with cross-compiled binaries; cross-linked from the umbrella release manifest |
...
The umbrella manifest builder iterates the canonical project list from
`docs/versioning.md`; `cli` joining that list means the builder picks
it up automatically. The CLI-specific shape in the manifest (`version`,
`release` URL, `assets` map) is already defined in the umbrella plan —
no further schema work is needed here.
...
**GoReleaser for cross-compile and per-component publish.** Smallest
viable pipeline for getting `.tar.gz`/`.zip` binaries onto a per-component
GitHub Release. Cosign-signed checksums plug into the same verification
story as the existing Docker images.

**The umbrella release owns `releases/latest` and the install URL; the
per-component CLI release is a binary holder.** The per-component
`cli-v<version>` release is published with `make_latest: false` so the
umbrella release's `install.sh` remains the canonical entry point.
Users who want a specific CLI version outside an umbrella release can
still download from the per-component release directly. This keeps the
CLI plan agnostic to installer mechanics — those live in
[`docs/releases.md`](../releases.md) — and means a CLI release that
isn't yet part of an umbrella release won't accidentally redirect
unrelated users.

**`cli-release-assets.json` is a workflow-to-workflow contract.** The
CLI publish workflow emits it; the umbrella publish workflow consumes
it. Schema lives in
[`umbrella-release-implementation.md`](./umbrella-release-implementation.md);
changes to it require coordinated PRs against both workflows.

**The CLI slots into the existing versioning machinery, not its own.**
A `cli/CHANGELOG.yml`, a `cli` case in `push-tag.yml`, and a
`publish-cli.yml` reusable workflow. Contributors don't learn a new
release flow.
...
- **Homebrew tap in v1, or follow-up?** A tap is cheap (a second small
  ...
  thing to maintain. The umbrella `install.sh` covers most users
  already, so deferring is defensible. Worth confirming before the
  release process is cemented so we don't re-architect later.
...
8. **Per-component release pipeline** — `.goreleaser.yaml` with
   `make_latest: false`, `publish-cli.yml` reusable workflow, the
   `cli` case in `push-tag.yml`, and `versioning.md` updated.
9. **Umbrella coordination** — add the post-GoReleaser step that
   produces `cli-release-assets.json` from `dist/artifacts.json` in the
   documented schema, attach it to the per-component release, upload
   it as a workflow artifact, and add `publish-cli` to the umbrella
   job's `needs:`. Validate end-to-end by triggering an umbrella
   `workflow_dispatch` against a real CLI release.
10. **E2E scenario** — extend `e2e/` to exercise the binary against a
    real orchestrator.
11. **Docs** — `cli/README.md`, `docs/cli.md` (usage only — installation
    lives in `docs/releases.md`), and root README updates.

Steps 1–6 can land behind a "not yet released" flag (i.e. no
`CHANGELOG.yml` bump) so the binary exists but isn't published while
the release pipeline is being built out. Step 9 depends on the
umbrella plan having landed at least up through its step 5 (digest
plumbing through `push-tag.yml`); coordinate with whoever owns that
work.
...
**`cli-release-assets.json` schema drift breaks the umbrella release.**
The CLI publish workflow produces the JSON; the umbrella publish
workflow consumes it; the schema lives in a third document. A change
on either side without the matching change on the other will fail the
umbrella build. Mitigation: the umbrella plan's builder script has
unit tests that assert against a vendored schema (section 1 of that
plan), and changes to the schema must update both workflows in the
same PR. The schema lives under version control, not in code comments.
...
linux/arm64, darwin/arm64), at least one team member runs a smoke
test manually before the first release of a major version. The
umbrella plan's install matrix exercises the same combos against the
generated `install.sh`, providing a second line of defence.

**Rollback story for a bad release.** Mitigation: a released binary
can't be unreleased, but the per-component release can be hidden and
a follow-up CLI version published. The umbrella release's
`install.sh` has the URLs baked in, so it points at whatever was
current when the umbrella was built — there is no auto-roll-forward.
Recovery is: ship a fixed CLI version, publish a new umbrella
...
- Add **`cli/README.md`** — quickstart for contributors (how to build
  ...
  subcommand).
- Add **`docs/cli.md`** — end-user **usage** (commands, flags, JSON
  output, exec streaming). Installation lives in
  [`docs/releases.md`](../releases.md), already merged; `docs/cli.md`
  links to it rather than duplicating.
- Update **`docs/versioning.md`** to add `cli` to the "Versioned
  projects" table, with the published-as column noting both the
  per-component release and the umbrella manifest cross-link.
...
The "GoReleaser + per-component release" decision doesn't need its own
ADR — it's a mechanical consequence of the umbrella-release ADR
([`2026-05-23-github-release-as-manifest.md`](../decisions/2026-05-23-github-release-as-manifest.md)),
which already establishes that components publish to their own
locations and the manifest cross-links to them.
