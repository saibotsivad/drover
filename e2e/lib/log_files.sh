...
#   assert_log_file_contains  "$WORKER_ID" "0.log" "expected substring"
...
assert_log_files_contains() {
	...
	api_get "${ORCHESTRATOR_URL}/workers/${cid}/logs/files"
...
assert_log_file_contains() {
	...
	api_get "${ORCHESTRATOR_URL}/workers/${cid}/logs/files/${name}"
