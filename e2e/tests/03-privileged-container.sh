...
#   5. WebSocket auth: no token / bad token / unknown container /
...
# output, and the per-container WebSocket endpoint in one pass. It only
...
#
# The per-container WS endpoint authenticates inline (HTTP middleware
...
ws_handshake "$WS_BASE/containers/${CONTAINER_ID}/ws"
...
ws_handshake "$WS_BASE/containers/${CONTAINER_ID}/ws?token=not-the-real-key"
assert_equals "403" "$WS_HANDSHAKE_STATUS" "bad ?token= rejected at handshake"
step_end

step_begin "ws-rejects-unknown-container"
ws_handshake "$WS_BASE/containers/cnt_does_not_exist/ws" \
	-H "Authorization: Bearer $DROVER_API_KEY"
assert_equals "403" "$WS_HANDSHAKE_STATUS" "unknown container rejected at handshake"
step_end

step_begin "ws-rejects-invalid-tail"
ws_handshake "$WS_BASE/containers/${CONTAINER_ID}/ws?tail=notanumber" \
	-H "Authorization: Bearer $DROVER_API_KEY"
...
ws_handshake "$WS_BASE/containers/${CONTAINER_ID}/ws?token=$DROVER_API_KEY"
...
ws_start_background "$WS_BASE/containers/${CONTAINER_ID}/ws" \
	-H "Authorization: Bearer $DROVER_API_KEY" \
...
wait_exec_complete "$CONTAINER_ID" "$WS_COMMAND_ID" 15 >/dev/null \
