...
func newStartCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Launch a micro-container (blocks until running by default)",
	...
	cmd.Flags().BoolVar(&privileged, "privileged", false, "run as a privileged micro-container")
	...
	cmd.Flags().IntVar(&timeout, "timeout", 0, "server-side container lifetime cap in seconds (0 = server default)")
	cmd.Flags().BoolVar(&noWait, "no-wait", false, "return as soon as the container is created")
